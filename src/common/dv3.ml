  ...
  let flush () = 
    with_cache.with_state (fun ~state ~set_state -> 
        ...
        begin
          ...
          | x when x=Flag.dirty -> 
            lower_ops.set_parent (parent|>fst)
        ...
        begin
          ...
          | x when x=Flag.clean -> return ()
          | x when x=Flag.dirty -> 
            lower_ops.set_times (times|>fst)
          | _ -> failwith "impossible"
        end >>= fun () -> 
        let cache = state.cache in
        cache_ops.clean cache |> fun (xs,cache) -> 
        lower_insert xs >>= fun () -> 
        (* NOTE a flush at this level causes a sync at the lower *)
        lower_ops.sync () >>= fun () -> 
        set_state {parent=clean state.parent; times=clean state.times; cache})

  let sync () = 
    flush () >>= fun () -> 
    (* lower_ops.sync () - no need - already synced by the flush    *)
    return ()
