...
module type S = sig
  ...
  type dir_entry = Fid of fid | Did of did | Sid of sid[@@deriving yojson]                  
  ...
  (** semantics: this initializes a new dir root_block, and inserts into the gom *)
(*
  type new_id_ops = {
    new_fid: unit -> (fid,t)m;
    (* new_did: parent:did -> name:str_256 -> (did*dir_ops,t)m; *)
    new_sid: unit -> (sid,t)m
  }
  val ids: new_id_ops
*)
...
(** NOTE use ids.new_did to create a new dir *)
  ...
  (* FIXME open minifs automatically? or at least some of the submodules within minifs_intf? *)
